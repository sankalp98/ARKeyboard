...
extension MainViewController: StreamDelegate {
    func stream(_ aStream: Stream, handle eventCode: Stream.Event) {
        ...
        inputStream.read(&bytes, maxLength: 52)
        ...
        DispatchQueue.main.async {
            ...
            let palmCenterPosition = SCNVector3(
                x: handData.xposition,
                y: handData.yposition,
                z: handData.zposition
            )
            let indexTipPosition = SCNVector3(
                x: handData.xindexTipPosition,
                y: handData.yindexTipPosition,
                z: handData.zindexTipPosition
            )
            
            let gesture = handData.gesture
            //print(translation.x, translation.y, translation.z)
            //self.sceneManager.updateGeometry(with: translation, and: rotation, and: indexTipPosition)
            self.sceneManager.matchCordinates(with: indexTipPosition, and: palmCenterPosition, and: gesture)
        }
    }
}
