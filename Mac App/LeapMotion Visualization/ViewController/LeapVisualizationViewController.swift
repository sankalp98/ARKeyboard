...
class LeapVisualizationViewController: NSViewController {
    ...
    fileprivate func stream(data: LMHData) {
        ...
        outputStream.write(data.toBytes(), maxLength: 52)
    }
...
//        let xindextipposition = handData.fingers[1].tipPosition.x
//        let yindextipposition = handData.fingers[1].tipPosition.y
//        let zindextipposition = handData.fingers[1].tipPosition.z
        let xindextipposition = handData.indexposition.x
        let yindextipposition = handData.indexposition.y
        let zindextipposition = handData.indexposition.z
        
        let gesture = Float(handData.gesture ?? 0)
        ...
        let serializedData = LMHData(
            ...
            xposition: handData.position.x,
            yposition: handData.position.y,
            zposition: handData.position.z,
            xindexTipPosition: xindextipposition,
            yindexTipPosition: yindextipposition,
            zindexTipPosition: zindextipposition,
            gesture: gesture
        )
        
        //print(yindextipposition)
