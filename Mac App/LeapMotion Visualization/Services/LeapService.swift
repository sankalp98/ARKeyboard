...
extension LeapService {
    ...
    func onConnect(_ notification: Notification!) {
        ...
        controller.setPolicy(LEAP_POLICY_OPTIMIZE_HMD)
    ...
    func onFrame(_ notification: Notification!) {
        guard
            ...
            let gestureslist = frame.gestures(frame),
        ...
        {
            var yogesture: Float
            let xgesture = gesture as! LeapGesture
            yogesture = Float(xgesture.type.rawValue)
            print(xgesture.type.rawValue)
            leapHandRepresentation?.gesture = yogesture
        }
        print(gestureslist.count)
        handRepresentation = leapHandRepresentation
    }
}
...
extension LeapHand {
    ...
    func getRepresentation() -> LeapHandRepresentation? {
        ...
        let normalizepalmPosition = frame.interactionBox()?.normalizePoint(palmPosition, clamp: false)
        let xnormalizepalmPosition = normalizepalmPosition?.x ?? 0
        let ynormalizepalmPosition = normalizepalmPosition?.y ?? 0
        let znormalizepalmPosition = normalizepalmPosition?.z ?? 0
        
        let position = SCNVector3(
            x: CGFloat(xnormalizepalmPosition).roundTo(places: roundingPlaces),
            y: CGFloat(ynormalizepalmPosition).roundTo(places: roundingPlaces),
            z: CGFloat(znormalizepalmPosition).roundTo(places: roundingPlaces)
        )
        let eulerAngles = SCNVector3(
        ...
        let indexFingerLeapVector = LeapVector(x: Float(fingerData[1].tipPosition.x), y: Float(fingerData[1].tipPosition.y), z: Float(fingerData[1].tipPosition.z))
        let normalizeIndexPosition = frame.interactionBox()?.normalizePoint(indexFingerLeapVector, clamp: false)
        let xnormalizeindexPosition = normalizeIndexPosition?.x ?? 0
        let ynormalizeindexPosition = normalizeIndexPosition?.y ?? 0
        let znormalizeindexPosition = normalizeIndexPosition?.z ?? 0
        
        let indexposition = SCNVector3(
            x: CGFloat(xnormalizeindexPosition).roundTo(places: roundingPlaces),
            y: CGFloat(ynormalizeindexPosition).roundTo(places: roundingPlaces),
            z: CGFloat(znormalizeindexPosition).roundTo(places: roundingPlaces)
        )
